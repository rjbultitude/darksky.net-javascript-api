...
( function ( root, factory ) {
    ...
    */
    ...
    function DarkSky( config ) {
        ...
        function getAPIUrl() {
            if (typeof config.PROXY_SCRIPT !== 'undefined') {
                if (validURL(config.PROXY_SCRIPT) === false) {
                    throw new Error('Invalid URL for proxy. Pass a valid URL or set the API_KEY');
                }
                return config.PROXY_SCRIPT + '?url=';
            } else if (config.API_KEY) {
                return 'https://api.darksky.net/forecast/' + config.API_KEY + '/';
            } else {
                throw new Error('No valid darksky API_KEY or PROXY_SCRIPT from host app or proxy');
            }
        }
    ...
    DarkSky.prototype.isJsonString = function isJsonString(str) {
        try {
            JSON.parse(str);
        } catch (e) {
            console.warn(e, 'invalid JSON. Check the proxy is working correctly');
            return false;
        }
        return true;
    }
    ...
    DarkSky.prototype.processData = function processData(forecasts, locations, fnKey, appCallback) {
        ...
        for ( var i = 0, length = forecasts.length; i < length; i++ ) {
            if (this.isJsonString(forecasts[i])) {
